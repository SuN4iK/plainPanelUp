...
void LauncherApplet::externalWidgetSetup() {
    ...
    if (mEntry.endsWith(".desktop")) {
        ...
        // --- Ищем .desktop-файл в правильном порядке ---
        ...
        // 2. Пользовательские приложения
            ...
            if (QFile::exists(homeDir + "/.local/share/applications/" + launcherData[1])) {
            desktopEntryPath = homeDir + "/.local/share/applications/" + launcherData[1];
            }
            // 3. Системные Flatpak (НОВЫЙ БЛОК)
            else if (QFile::exists("/var/lib/flatpak/exports/share/applications/" + launcherData[1])) {
            desktopEntryPath = "/var/lib/flatpak/exports/share/applications/" + launcherData[1];
            }
            // 4. Пользовательские Flatpak (НОВЫЙ БЛОК)
            else if (QFile::exists(homeDir + "/.local/share/flatpak/exports/share/applications/" + launcherData[1])) {
            desktopEntryPath = homeDir + "/.local/share/flatpak/exports/share/applications/" + launcherData[1];
            }
        }
