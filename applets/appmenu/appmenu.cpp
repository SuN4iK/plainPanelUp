...
void AppMenuApplet::buildMenu(QListWidget* appsList, QString filter) {
    // Добавляем пути к иконкам Flatpak в поисковые пути Qt
    QStringList iconPaths = QIcon::themeSearchPaths();
    iconPaths << "/var/lib/flatpak/exports/share/icons"
    ...
    // --- Системные приложения (/usr/share/applications) ---
    ...
    // --- Пользовательские приложения (~/.local/share/applications) ---
    ...
    // --- Системные Flatpak (/var/lib/flatpak/exports/share/applications) ---
    appDir.setPath("/var/lib/flatpak/exports/share/applications");
    if (appDir.exists()) {
        desktopEntriesList = appDir.entryList(QDir::Files | QDir::NoDotAndDotDot);
        for (const QString &entry : desktopEntriesList) {
            if (entry.endsWith(".desktop")) {
                currentApp = readDesktopEntry(appDir.absoluteFilePath(entry));
                if (currentApp.display && currentApp.displayedName.contains(filter, Qt::CaseInsensitive)) {
                    QListWidgetItem* item = new QListWidgetItem(currentApp.displayedName);
                    item->setIcon(currentApp.icon.isNull() ? QIcon::fromTheme("dialog-question") : currentApp.icon);
                    appsList->addItem(item);
                    mExecByItem[item] = currentApp.exec;
                }
            }
    ...
    // --- Пользовательские Flatpak (~/.local/share/flatpak/exports/share/applications) ---
    appDir.setPath(homeDir + "/.local/share/flatpak/exports/share/applications");
    if (appDir.exists()) {
        desktopEntriesList = appDir.entryList(QDir::Files | QDir::NoDotAndDotDot);
        for (const QString &entry : desktopEntriesList) {
            if (entry.endsWith(".desktop")) {
...
void AppMenuApplet::buildFavMenu(ConfigManager* cfgMan, QListWidget* favAppsList) {
    ...
    // Добавляем пути иконок Flatpak (на случай, если buildMenu не вызывался перед этим)
    QStringList iconPaths = QIcon::themeSearchPaths();
    iconPaths << "/var/lib/flatpak/exports/share/icons"
              << QDir::homePath() + "/.local/share/flatpak/exports/share/icons";
    QIcon::setThemeSearchPaths(iconPaths);
    ...
    QDir globalFlatpakAppDir("/var/lib/flatpak/exports/share/applications");
    QDir localFlatpakAppDir(homeDir + "/.local/share/flatpak/exports/share/applications");
    ...
    QStringList globalFlatpakDesktopEntriesList = globalFlatpakAppDir.entryList(QDir::Files | QDir::NoDotAndDotDot);
    QStringList localFlatpakDesktopEntriesList = localFlatpakAppDir.entryList(QDir::Files | QDir::NoDotAndDotDot);
    ...
    for (int i = 0; i < cfgMan->mFavApps.count(); ++i) {
        ...
        else if (globalFlatpakDesktopEntriesList.contains(favDesktopEntry)) {
            desktopEntryPath = globalFlatpakAppDir.absoluteFilePath(favDesktopEntry);
        }
        else if (localFlatpakDesktopEntriesList.contains(favDesktopEntry)) {
            desktopEntryPath = localFlatpakAppDir.absoluteFilePath(favDesktopEntry);
        }
        else {
            continue; // не найдено – пропускаем
        }
